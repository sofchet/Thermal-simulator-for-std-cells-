...
os.system('/tools/OpenROAD/alpha-release/openroad/OpenROAD-2019-07-30_05-17/bin/resizer -exit script')
...
for i in range(3,len(stamylines)):
	internalpower.append(stamylines[i][0])
	leakagepower.append(stamylines[i][2])
...
for i in range(len(stapower)):
	st=stapower[i]
	st='%s %s %s %s %s \n'%(st,xcoord[inst.index(stainst[i])],ycoord[inst.index(stainst[i])],internalpower[i],leakagepower[i])
